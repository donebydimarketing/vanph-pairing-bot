...
const VERIFIED_ROLE_NAME = 'Verified'; // role granted after successful email match
...
// Grants the "Verified" role to a member once their email is successfully matched.
async function grantVerifiedRole(discordUserId) {
  try {
    const guild = await client.guilds.fetch(GUILD_ID);
    const role = guild.roles.cache.find((r) => r.name === VERIFIED_ROLE_NAME);
    if (!role) {
      console.error(`Role "${VERIFIED_ROLE_NAME}" not found in guild. Did you create it?`);
      return false;
    }
    ...
    console.error(`Failed to grant Verified role to ${discordUserId}:`, err.message);
    return false;
  }
}
...
client.on('messageCreate', async (message) => {
  ...
  if (memberMatch) {
    ...
    await grantVerifiedRole(discordUserId);
    await message.reply(`You're linked, ${memberMatch.full_name || 'there'}! ✅\n\nYou now have access to the server. You'll be added to a private channel automatically once a pairing is confirmed.`);
